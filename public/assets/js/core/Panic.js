function Panic(token) {
    ...
    this.build = function (ticket, order = 'append') {
        ...
        var card = '<div class="card pr-0 card-inline card-hover" data-panic-id="'+ticket.ticket_id+'">' +
            '<div class="card-content">' +
            '<div class="card-body">' +
            '<div class="card-text">' +
            '<h3>Ticket #'+ticket.ticket_id+'</h3>' +
            '<p>'+ticket.name+'</p>' +
            '<a href="#!" data-panic-id="'+ticket.ticket_id+'">Clique para ver mais <i class="mdi mdi-open-in-new"></i></a>' +
            '</div>' +
            '</div>' +
            '</div>' +
            '</div>'; 
